...
async function getOrCreatePlanId(razorpay, planKey) {
  ...
  const created = await razorpay.plans.create({ period: plan.period, interval: plan.interval, item: { name: plan.label, amount: plan.amount, currency: plan.currency, description: "Auto Google Review Responder" } });
...
exports.createSubscription = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const subscription = await razorpay.subscriptions.create({ plan_id: planId, customer_notify: 1, quantity: 1, total_count: cycle === "annual" ? 10 : 120, notes: { vendorId, vendorName: vendorName || "", vendorEmail: vendorEmail || "", source: "stall-app", planKey } });
    await db.collection("premium_vendors").doc(vendorId).set({ isPremium: false, subscriptionId: subscription.id, planId, planKey, billingCycle: cycle, amount: plan.amount, currency: plan.currency, status: "created", vendorName: vendorName || "", vendorEmail: vendorEmail || "", createdAt: admin.firestore.FieldValue.serverTimestamp(), activatedAt: null, nextBillingDate: null, payments: [] }, { merge: true });
    const cfg = functions.config().razorpay;
...
exports.verifySubscription = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    await db.collection("premium_vendors").doc(vendorId).set({ isPremium: true, status: "active", subscriptionId: razorpay_subscription_id, activatedAt: admin.firestore.FieldValue.serverTimestamp(), nextBillingDate: nextBilling, payments: admin.firestore.FieldValue.arrayUnion({ paymentId: razorpay_payment_id, amount: payment.amount, paidAt: admin.firestore.Timestamp.now(), method: payment.method }) }, { merge: true });
  ...
  try {
    switch (event) {
      case "subscription.charged": {
        const subscription = payload.subscription?.entity, payment = payload.payment?.entity;
        if (!subscription) break;
        const snap = await db.collection("premium_vendors").where("subscriptionId", "==", subscription.id).limit(1).get();
        if (snap.empty) break;
        const vendorId = snap.docs[0].id, existingData = snap.docs[0].data();
        const nextBilling = new Date(); nextBilling.setDate(nextBilling.getDate() + (existingData.billingCycle === "annual" ? 365 : 30));
        await db.collection("premium_vendors").doc(vendorId).update({ isPremium: true, status: "active", nextBillingDate: nextBilling, payments: admin.firestore.FieldValue.arrayUnion({ paymentId: payment?.id || "", amount: payment?.amount || existingData.amount || 49900, paidAt: admin.firestore.Timestamp.now(), method: payment?.method || "auto" }) });
        console.log(`✅ Subscription renewed for vendor ${vendorId}`); break;
      }
      case "subscription.payment.failed":
      case "payment.failed": {
        const subscription = payload.subscription?.entity; if (!subscription) break;
        const snap = await db.collection("premium_vendors").where("subscriptionId", "==", subscription.id).limit(1).get(); if (snap.empty) break;
...
exports.beginGbpOauth = functions.https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError("unauthenticated", "Login required");
  const vendorId = context.auth.uid, state = crypto.randomBytes(24).toString("hex");
  await db.collection("oauth_states").doc(state).set({ vendorId, createdAt: admin.firestore.FieldValue.serverTimestamp(), expiresAt: new Date(Date.now() + 10 * 60 * 1000) });
  return { state };
});
...
exports.oauthCallback = functions.https.onRequest(async (req, res) => {
  ...
  try {
    const stateRef = db.collection("oauth_states").doc(state), stateDoc = await stateRef.get();
...
// Google Reviews uses the full resource name accounts/{account}/locations/{location}.
// Support both the new stored format and existing connections that only stored locations/{id}.
function getReviewResourceName(connectionData) {
  const stored = String(connectionData.locationId || "").replace(/^\/+|\/+$/g, "");
  if (stored.startsWith("accounts/")) return stored;
  const account = String(connectionData.accountName || "").replace(/^\/+|\/+$/g, "");
  if (!account || !stored) throw new Error("GBP account/location information is missing");
...
exports.pollReviews = functions.pubsub.schedule("every 30 minutes").onRun(async () => {
  ...
  const promises = connectionsSnap.docs.map(async (connDoc) => {
    ...
    try {
      ...
      const resourceName = getReviewResourceName(connectionData);
      const reviewsRes = await axios.get(`https://mybusiness.googleapis.com/v4/${resourceName}/reviews`, { headers: { Authorization: `Bearer ${accessToken}` }, params: { pageSize: 50 } });
      ...
      for (const review of reviews) {
        ...
        await axios.put(`https://mybusiness.googleapis.com/v4/${resourceName}/reviews/${reviewId}/reply`, { comment: aiResponse }, { headers: { Authorization: `Bearer ${accessToken}` } });
...
exports.triggerPollForVendor = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const accessToken = await getValidToken(vendorId, connectionData);
    const resourceName = getReviewResourceName(connectionData);
    const reviewsRes = await axios.get(`https://mybusiness.googleapis.com/v4/${resourceName}/reviews`, { headers: { Authorization: `Bearer ${accessToken}` }, params: { pageSize: 10 } });
    ...
    console.error("triggerPollForVendor error:", err.response?.status, err.response?.data || err.message);
    throw new functions.https.HttpsError("unavailable", "Google review sync failed. Please check Google Business Profile API access and try again.");
  }
});
...
exports.weeklyCustomerDigest = functions.pubsub.schedule("every monday 09:00").timeZone("Asia/Kolkata").onRun(async () => {
  console.log("weeklyCustomerDigest: starting");
  const vendorsSnap = await db.collection("vendors").get();
  const vendors = vendorsSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
  const weekAgo = Date.now() - 7 * 24 * 60 * 60 * 1000;
  const newVendors = vendors.filter((v) => { const created = v.createdAt?.toDate?.(); return created && created.getTime() >= weekAgo; });
  const activeOffers = vendors.filter((v) => { if (!v.offer) return false; const exp = v.offerExpiresAt?.toDate?.(); return !exp || exp.getTime() >= Date.now(); });
  const usersSnap = await db.collection("users").get(); let digestsWritten = 0;
  for (const userDoc of usersSnap.docs) {
    const uid = userDoc.id, favSnap = await db.collection("users").doc(uid).collection("favorites").get(); if (favSnap.empty) continue;
    const favIds = new Set(favSnap.docs.map((d) => d.id)); const favVendors = vendors.filter((v) => favIds.has(v.id) && typeof v.lat === "number" && typeof v.lng === "number"); if (!favVendors.length) continue;
    const centroid = { lat: favVendors.reduce((s, v) => s + v.lat, 0) / favVendors.length, lng: favVendors.reduce((s, v) => s + v.lng, 0) / favVendors.length };
    const nearbyNew = newVendors.filter((v) => typeof v.lat === "number" && typeof v.lng === "number" && haversineKm(centroid, v) <= DIGEST_RADIUS_KM);
    const nearbyOffers = activeOffers.filter((v) => typeof v.lat === "number" && typeof v.lng === "number" && haversineKm(centroid, v) <= DIGEST_RADIUS_KM);
    if (!nearbyNew.length && !nearbyOffers.length) continue;
    await db.collection("digests").doc(uid).set({ weekOf: admin.firestore.FieldValue.serverTimestamp(), newVendors: nearbyNew.map((v) => ({ id: v.id, name: v.name, category: v.category })), activeOffers: nearbyOffers.map((v) => ({ id: v.id, name: v.name, offer: v.offer })), read: false });
    digestsWritten++;
  }
  console.log(`weeklyCustomerDigest: wrote ${digestsWritten} digests`); return null;
});

exports.weeklyVendorDigest = functions.pubsub.schedule("every monday 09:00").timeZone("Asia/Kolkata").onRun(async () => {
  console.log("weeklyVendorDigest: starting");
  const vendorsSnap = await db.collection("vendors").get(); const claimedVendors = vendorsSnap.docs.filter((d) => d.data().ownerId); let digestsWritten = 0;
  for (const vendorDoc of claimedVendors) {
    const v = vendorDoc.data(), vendorId = vendorDoc.id, snapshotRef = db.collection("vendor_counter_snapshots").doc(vendorId), prevSnap = await snapshotRef.get();
    const prev = prevSnap.exists ? prevSnap.data() : { viewCount: 0, callCount: 0, whatsappCount: 0, directionsCount: 0 };
    const current = { viewCount: v.viewCount || 0, callCount: v.callCount || 0, whatsappCount: v.whatsappCount || 0, directionsCount: v.directionsCount || 0 };
    const delta = { views: Math.max(0, current.viewCount - (prev.viewCount || 0)), calls: Math.max(0, current.callCount - (prev.callCount || 0)), whatsapp: Math.max(0, current.whatsappCount - (prev.whatsappCount || 0)), directions: Math.max(0, current.directionsCount - (prev.directionsCount || 0)) };
    await snapshotRef.set(current); if (delta.views + delta.calls + delta.whatsapp + delta.directions === 0) continue;
    await db.collection("vendor_digests").doc(vendorId).set({ weekOf: admin.firestore.FieldValue.serverTimestamp(), vendorName: v.name, ...delta, read: false }); digestsWritten++;
  }
  console.log(`weeklyVendorDigest: wrote ${digestsWritten} digests`); return null;
});
...
async function fetchBoostData(accessToken, connectionData) {
  ...
  let mediaItems = [];
  try { const mediaRes = await axios.get(`https://mybusinessbusinessinformation.googleapis.com/v1/${locationId}/media`, { headers: { Authorization: `Bearer ${accessToken}` } }); mediaItems = mediaRes.data.mediaItems || []; } catch (err) { mediaItems = []; }
...
exports.runBoostScan = functions.https.onCall(async (data, context) => {
  if (!context.auth) throw new functions.https.HttpsError("unauthenticated", "Login required");
  const vendorId = context.auth.uid;
  const [connDoc, premiumDoc, vendorSnap] = await Promise.all([db.collection("gbp_connections").doc(vendorId).get(), db.collection("premium_vendors").doc(vendorId).get(), db.collection("vendors").where("ownerId", "==", vendorId).limit(1).get()]);
  if (!connDoc.exists || !connDoc.data().connected) throw new functions.https.HttpsError("failed-precondition", "GBP not connected");
  if (!premiumDoc.exists || !premiumDoc.data().isPremium) throw new functions.https.HttpsError("failed-precondition", "Premium subscription required");
  ...
  try {
    ...
    const { score, band, checklist } = scoreProfile(profile); const vendorCopy = await writeVendorFacingCopy(checklist, listing.name);
  ...
  } catch (err) { console.error("runBoostScan failed:", err.response?.status, err.response?.data || err.message); throw new functions.https.HttpsError("unavailable", "Couldn't complete the scan. Please try again in a moment."); }
