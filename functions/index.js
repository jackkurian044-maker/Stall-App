        ...
        if (vendorSnap?.exists && vendorSnap.data().ownerId === vendorId) {
          ...
          try {
            ...
            if (!commissionSnap.empty) {
              ...
              const daysSinceConversion = convertedAt ? (Date.now() - convertedAt.getTime()) / (1000 * 60 * 60 * 24) : null;
              if (commissionData.status !== "clawed_back" && daysSinceConversion !== null && daysSinceConversion <= 30) {
                await commissionDoc.ref.update({ status: "clawed_back", clawedBackAt: admin.firestore.FieldValue.serverTimestamp() });
          ...
          } catch (clawErr) { console.error("Clawback check error:", clawErr); }
        }
...
exports.pollReviews = functions.pubsub.schedule("every 30 minutes").onRun(async () => {
  ...
  const promises = connectionsSnap.docs.map(async (connDoc) => {
    ...
    try {
      const premiumDoc = await db.collection("premium_vendors").doc(vendorId).get();
      if (!premiumDoc.exists || !premiumDoc.data().isPremium) { console.log(`Skipping vendor ${vendorId} — not premium`); return; }
      ...
      for (const review of reviews) {
        ...
        if (existingDoc.exists || review.reviewReply) continue;
...
exports.weeklyCustomerDigest = functions.pubsub.schedule("every monday 09:00").timeZone("Asia/Kolkata").onRun(async () => {
  ...
  for (const userDoc of usersSnap.docs) {
    const uid = userDoc.id; const favSnap = await db.collection("users").doc(uid).collection("favorites").get(); if (favSnap.empty) continue;
    const favIds = new Set(favSnap.docs.map((d) => d.id)); const favVendors = vendors.filter((v) => favIds.has(v.id) && typeof v.lat === "number" && typeof v.lng === "number"); if (favVendors.length === 0) continue;
    ...
    await db.collection("digests").doc(uid).set({ weekOf: admin.firestore.FieldValue.serverTimestamp(), newVendors: nearbyNew.map((v) => ({ id: v.id, name: v.name, category: v.category })), activeOffers: nearbyOffers.map((v) => ({ id: v.id, name: v.name, offer: v.offer })), read: false }); digestsWritten++;
  }
...
exports.weeklyVendorDigest = functions.pubsub.schedule("every monday 09:00").timeZone("Asia/Kolkata").onRun(async () => {
  ...
  for (const vendorDoc of claimedVendors) {
    const v = vendorDoc.data(); const vendorId = vendorDoc.id; const snapshotRef = db.collection("vendor_counter_snapshots").doc(vendorId); const prevSnap = await snapshotRef.get(); const prev = prevSnap.exists ? prevSnap.data() : { viewCount: 0, callCount: 0, whatsappCount: 0, directionsCount: 0 };
...
exports.runBoostScan = functions.https.onCall(async (data, context) => {
  ...
  const vendorDoc = vendorSnap.docs[0]; const listing = vendorDoc.data(); const connectionData = connDoc.data(); let result;
  try {
    const accessToken = await getValidToken(vendorId, connectionData);
...
exports.testGrowthSetupEntitlement = functions.https.onCall(async (data, context) => {
  try {
    ...
    console.error("testGrowthSetupEntitlement failed:", err);
    if (err instanceof functions.https.HttpsError) throw err;
    throw new functions.https.HttpsError("internal", err?.message || "Could not enable test entitlement");
  }
});
...
exports.revertTestGrowthSetupEntitlement = functions.https.onCall(async (data, context) => {
  try {
    ...
    restored.basePlanKey = backup.basePlanKey
      ? backup.basePlanKey
      : admin.firestore.FieldValue.delete();

    await listingRef.set(restored, { merge: true });
    return { success: true, listingId, reverted: true };
  } catch (err) {
    console.error("revertTestGrowthSetupEntitlement failed:", err);
    if (err instanceof functions.https.HttpsError) throw err;
