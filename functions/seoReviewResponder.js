...
const DEFAULT_SETTINGS = {
  autoReply: true, seoOptimization: true, tone: "friendly", language: "english", signOff: "", customInstructions: "",
  replyTo1Star: true, replyTo2Star: true, replyTo3Star: true, replyTo4Star: true, replyTo5Star: true,
  responseLength: "standard", serviceKeywords: [], locationKeywords: [],
};
...
function getGoogleOAuthConfig() {
  let cfg; try { cfg = JSON.parse(googleOAuthConfig.value()); } catch (err) { throw new Error("GOOGLE_OAUTH_CONFIG is missing or invalid"); }
  if (!cfg.client_id || !cfg.client_secret || !cfg.redirect_uri) throw new Error("GOOGLE_OAUTH_CONFIG is missing client_id, client_secret, or redirect_uri");
  return cfg;
}
async function getVertexAccessToken() {
  const credential = admin.app().options.credential;
  if (!credential || typeof credential.getAccessToken !== "function") throw new Error("Firebase Admin credential cannot provide a Google access token");
...
async function refreshAccessToken(vendorId, connectionData) {
  ...
  const tokenRes = await axios.post("https://oauth2.googleapis.com/token", { refresh_token: connectionData.refreshToken, client_id: cfg.client_id, client_secret: cfg.client_secret, grant_type: "refresh_token" });
  const { access_token, expires_in } = tokenRes.data;
  if (!access_token) throw new Error("Google did not return a refreshed access token");
  await db.collection("gbp_connections").doc(vendorId).update({ accessToken: access_token, tokenExpiresAt: new Date(Date.now() + Number(expires_in || 3600) * 1000) });
  return access_token;
}
async function getValidToken(vendorId, connectionData) {
  const expiry = connectionData.tokenExpiresAt?.toDate?.() || new Date(0);
  return expiry < new Date(Date.now() + 5 * 60 * 1000) || !connectionData.accessToken ? refreshAccessToken(vendorId, connectionData) : connectionData.accessToken;
}
function buildSeoGuidance(listing, settings) {
  ...
  const listingServices = Array.isArray(listing?.services) ? listing.services.map(s => typeof s === "string" ? s : s?.name).filter(Boolean).slice(0, 4) : [];
  const services = settings.serviceKeywords.length ? settings.serviceKeywords : listingServices;
  const locations = settings.locationKeywords.length ? settings.locationKeywords : (address ? [address] : []);
  return { name, category, address, serviceText: services.length ? services.join(", ") : category, locationText: locations.join(", ") };
}
async function generateSeoResponse(review, listing, rawSettings) {
  ...
  const response = await axios.post(endpoint, { contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: { maxOutputTokens: 260, temperature: 0.35 } }, { headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" } });
...
function reviewUrl(connectionData, reviewId) { return `https://mybusiness.googleapis.com/v4/${connectionData.accountName}/${connectionData.locationId}/reviews/${reviewId}`; }
async function rewriteOne(vendorId, connectionData, reviewId, existingReview) {
  ...
  const review = { reviewerName: latestReview.reviewer?.displayName || existingReview?.reviewerName || "Valued Customer", reviewText: latestReview.comment || existingReview?.reviewText || "", starRating: ratingMap[latestReview.starRating] || existingReview?.starRating || 3 };
...
exports.rewriteReviewResponse = functions.runWith({ secrets: [googleOAuthConfig] }).https.onCall(async (data, context) => {
  ...
  const vendorId = context.auth.uid; const reviewId = String(data?.reviewId || "").trim();
  if (!reviewId) throw new functions.https.HttpsError("invalid-argument", "Review ID is required");
  try {
    const [connDoc, reviewDoc] = await Promise.all([db.collection("gbp_connections").doc(vendorId).get(), db.collection("review_responses").doc(`${vendorId}_${reviewId}`).get()]);
...
exports.pollReviews = functions.runWith({ secrets: [googleOAuthConfig] }).pubsub.schedule("every 30 minutes").timeZone("Asia/Kolkata").onRun(async () => {
  ...
  for (const connDoc of connectionsSnap.docs) {
    const vendorId = connDoc.id; const connectionData = connDoc.data();
    try {
      const premiumDoc = await db.collection("premium_vendors").doc(vendorId).get();
      if (!premiumDoc.exists || !premiumDoc.data().isPremium) continue;
      const settings = normalizeSettings(connectionData.responseSettings);
      if (!settings.autoReply) { await connDoc.ref.set({ lastPolled: admin.firestore.FieldValue.serverTimestamp(), responderStatus: "disabled" }, { merge: true }); continue; }
      const vendorSnap = await db.collection("vendors").where("ownerId", "==", vendorId).limit(1).get();
      const listing = vendorSnap.docs[0]?.data() || {};
      const accessToken = await getValidToken(vendorId, connectionData);
      // Only inspect the newest 10 reviews. Google returns them newest first.
      // We intentionally do not import the full historical review list.
      const reviewsRes = await axios.get(`https://mybusiness.googleapis.com/v4/${connectionData.accountName}/${connectionData.locationId}/reviews`, { headers: { Authorization: `Bearer ${accessToken}` }, params: { pageSize: 10, orderBy: "updateTime desc" } });
      ...
      for (const review of reviews) {
        ...
        const existing = await ref.get(); const existingData = existing.exists ? existing.data() : {};
        ...
        try {
          ...
          if (latest.data.reviewReply) { await ref.delete(); continue; }
          await axios.put(`${reviewUrl(connectionData, reviewId)}/reply`, { comment: aiResponse }, { headers: { Authorization: `Bearer ${accessToken}` } });
      ...
      await connDoc.ref.set({ lastPolled: admin.firestore.FieldValue.serverTimestamp(), lastReviewSyncCount: newUnanswered, totalReviewCount, averageRating, responderStatus: "active" }, { merge: true });
    } catch (err) { console.error(`SEO poll failed for vendor ${vendorId}:`, err.response?.status, err.response?.data || err.message); }
  }
  return null;
});
