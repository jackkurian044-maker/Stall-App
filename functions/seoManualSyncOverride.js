...
async function reworkLatestPosted(vendorId, connection, listing) {
  const postedSnap = await db.collection("review_responses").where("vendorId", "==", vendorId).get();
  const posted = postedSnap.docs.map(d => ({ ref: d.ref, data: d.data() })).filter(x => x.data.status === "posted").sort((a, b) => (b.data.postedAt?.toMillis?.() || 0) - (a.data.postedAt?.toMillis?.() || 0));
  ...
  const latest = await axios.get(reviewResource(connection, reviewId), { headers: { Authorization: `Bearer ${accessToken}` } });
  const googleReview = latest.data;
  if (!googleReview) throw new Error("Google review could not be re-read");
  const ratingMap = { ONE: 1, TWO: 2, THREE: 3, FOUR: 4, FIVE: 5 };
  const review = { reviewerName: googleReview.reviewer?.displayName || existing.reviewerName || "Valued Customer", reviewText: googleReview.comment || existing.reviewText || "", starRating: ratingMap[googleReview.starRating] || existing.starRating || 3 };
  const effectiveListing = { ...listing, name: listing.name || connection.locationName || "Our Business" };
  const aiResponse = await responder.generateSeoResponse(review, effectiveListing, settings);
  await axios.put(`${reviewResource(connection, reviewId)}/reply`, { comment: aiResponse }, { headers: { Authorization: `Bearer ${accessToken}` } });
  await ref.set({ reviewerName: review.reviewerName, reviewText: review.reviewText, starRating: review.starRating, aiResponse, googleReply: aiResponse, status: "posted", seoOptimized: !!settings.seoOptimization, seoVersion: 6, reworkedAt: admin.firestore.FieldValue.serverTimestamp(), syncedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
...
exports.triggerPollForVendor = functions.runWith({ secrets: [googleOAuthConfig] }).https.onCall(async (data, context) => {
  ...
  try {
    ...
    const reviewParent = locationResource.includes("/locations/") ? locationResource : `${accountResource}/locations/${locationResource.replace(/^locations\//, "")}`;
    const reviewsRes = await axios.get(`https://mybusiness.googleapis.com/v4/${reviewParent}/reviews`, { headers: { Authorization: `Bearer ${accessToken}` }, params: { pageSize: 10, orderBy: "updateTime desc" } });
    ...
    const ratingMap = { ONE: 1, TWO: 2, THREE: 3, FOUR: 4, FIVE: 5 };
    let processedCount = 0, skippedCount = 0, errorCount = 0, unansweredFound = 0;
    for (const review of reviews) {
      ...
      try {
        const effectiveListing = { ...listing, name: listing.name || connection.locationName || "Our Business" };
        const aiResponse = await responder.generateSeoResponse({ reviewerName: review.reviewer?.displayName || "Valued Customer", reviewText: review.comment || "", starRating }, effectiveListing, settings);
        const latest = await axios.get(responder.reviewUrl(connection, reviewId), { headers: { Authorization: `Bearer ${accessToken}` } });
        if (latest.data.reviewReply) { await ref.delete(); skippedCount++; continue; }
        ...
        await ref.set({ aiResponse, googleReply: aiResponse, status: "posted", seoOptimized: !!settings.seoOptimization, seoVersion: 6, postedAt: admin.firestore.FieldValue.serverTimestamp(), processingAt: admin.firestore.FieldValue.delete(), syncedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        processedCount++;
      } catch (err) {
        errorCount++;
