...
export default function VendorSuccessPanel({ listings, onEdit, onOffer, onTab }) {
  ...
  const items = [
    ...
    { key: "direct", label: "Direct business", score: (listing.planKey === "growth_setup" || (listing.isPremium && listing.subscriptionTier === "growth_setup") || listing.testGrowthSetupActive === true || listing.testGrowthSetupActive === true) ? 10 : 0, max: 10, good: listing.planKey === "growth_setup" || (listing.isPremium && listing.subscriptionTier === "growth_setup") || listing.testGrowthSetupActive === true || listing.testGrowthSetupActive === true, title: "Take direct orders & bookings", text: "If your plan includes STall Direct, orders and bookings can run through your own STall storefront.", button: "Open STall Direct", icon: <BarChart2 size={14} />, run: () => onTab("direct") },
  ];
        ...
        {improvement && (
          <div style={{ marginTop: 11 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 8 }}>
              {[0, 1, 2].map((index) => {
                const image = improvement.images?.[index];
                if (!image) return null;
                return (
                  <div key={index} style={{ minWidth: 0, border: "1px solid #ddd", borderRadius: 9, overflow: "hidden", background: "#fff" }}>
                    ...
                    <div style={{ padding: 7, fontSize: 9.5, fontWeight: 800, color: image.kind === "stall" ? COLORS.teal : "#777" }}>
                      {image.label || "Prepared creative"}{image.kind === "stall" ? " • promotion interstitial" : " • category/service"}
                    </div>
                  </div>
                );
              })}
